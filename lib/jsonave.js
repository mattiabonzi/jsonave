  ...
  toResult: function (options) {
    ...
    if (result.length === 1 && !options.wrap && !Array.isArray(result[0].value)) {
      if (options.resultType === 'all') {
        return {...result[0].value, path: asPath(result[0].path)};
      }
    ...
    return result.reduce(function (result, ea) {
      ...
      if (options.resultType === 'all') {
        valOrPath = {...ea.value, path: asPath(ea.path)};;
      }
...
var instance = exports.instance = function (inputExpr, opts) {
  ...
  if (!['all', 'value', 'path', 'patharray'].includes(opts.resultType)) {
    throw new Error('Invalid option resultType: ' + opts.resultType);
  }

  opts.pathNeeded = ['all',  'path', 'patharray'].includes(opts.resultType);
